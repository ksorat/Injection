...
vi = 1
T0 = 0.0
title = 'Equatorial and Meridional Slices'
tmpvid = 'tmpVid1'
...
# legends
plXs = [0.03]
plYs = [0.0,0.0,0.9,0.9,0.9,0.55]
plTits = ["", "", varTitles[vi], varTitles[vi], varTitles[vi], ""]

# dbs[0] = hemisphere1.stl file
# dbs[1] = hemisphere2.stl file
# dbs[2] = r/phi xmf file
# dbs[3] = first r/theta xmf file
# dbs[4] = second r/theta xmf file
sphereDir = os.path.dirname(os.path.realpath(__file__))
dbs = [sphereDir + '/hemisphere1.stl', sphereDir + '/hemisphere2.stl', sys.argv[1], sys.argv[2], sys.argv[3]]
...
pyv.lfmPCol(dbs[3], varNames[vi], vBds=bnds, Log=True, cMap='plasma', Light=True, Legend=False)
pyv.addThreshold(varNames[vi], bnds[0], 1e10)
pyv.addThreshold("Theta", 0, 90, opNum=1)
pyv.lfmPCol(dbs[4], varNames[vi], vBds=bnds, Log=True, cMap='plasma', Light=True, Legend=False)
pyv.addThreshold(varNames[vi], bnds[0], 1e10)
pyv.addThreshold("Theta", 0, 90, opNum=1)
...
pyv.genPolarCoords2(dbs[3], 2, 10, 45, aType='theta', phi=90, contours=True)
pyv.genPolarCoords2(dbs[4], 2, 10, 45, aType='theta', phi=270, contours=True)
...
tit = pyv.genTit( titS=title, Pos=(0.2,0.955) )
...
outVid = "vid_r_theta_r_phi_" + varNames[vi] + "_all.mp4"
